-<?php 
  ...
  if(isset($_POST['submit'])){
    ...
    if(!empty($_POST['name']) && !empty($_POST['email']) && !empty($_POST['password']) && !empty($_POST['gender']) && !empty($_POST['mobile']) && !empty($_POST['thana'])){
        ...
        $query = "insert into incharge(name, email, password, gender, mobile, tid) values('$name', '$email', '$password', '$gender', '$mobile', '$thana')";

        $run = mysqli_query($conn, $query) or die(mysqli_error());

        if($run){
            echo "Incharge is successfully added!";
        }
        else{
...
<div>
  ...
  <div class="login-page">
      <h3 style="text-align:center; color: white;">Incharge Registration</h3> <br>
        <div class="form">
          <form method="post" action="add incharge.php">
